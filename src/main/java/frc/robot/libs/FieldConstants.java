...
public class FieldConstants {
  ...
  public enum AprilTagLayoutType {
    ...
    public static boolean isInTowerZone(Translation2d point) {
        Translation2d blueCenter = FieldConstants.Tower.centerPoint.minus(new Translation2d(FieldConstants.Tower.depth / 2.0, 0.0));

        // Mirror for red side
        Translation2d redCenter = new Translation2d(
            FieldConstants.fieldLength - blueCenter.getX(),
            FieldConstants.fieldWidth - blueCenter.getY()
        );

        double halfWidth = FieldConstants.Tower.width / 2.0;
        double halfDepth = FieldConstants.Tower.depth / 2.0;

        // Check blue tower
        boolean inBlue =
            Math.abs(point.getX() - blueCenter.getX()) < halfDepth &&
            Math.abs(point.getY() - blueCenter.getY()) < halfWidth;

        // Check red tower
        boolean inRed =
            Math.abs(point.getX() - redCenter.getX()) < halfDepth &&
            Math.abs(point.getY() - redCenter.getY()) < halfWidth;
